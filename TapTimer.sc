TapTimer {
	...
	var clock, last_time, <phrase_len, <tempo, <beats_per_phrase, <>mIN_LEN,
		mAX_LEN, timearr, <>error_margin, pr_beat_flag, <>action;
	...
	*new { arg max = 16, phrase_len = 4, beats_per_phrase = 4, error_margin = 0.1,
	...
	tap 	{ arg time;
		...
		"checked time for nil".postln;

		last_time.isNil.if({ last_time = 0});
		"checked last time for nil".postln;

		(last_time == 0). if ({
			last_time = time;
			"last_time was zero".postln;
			} , {
		...
		return_val.notNil.if({
			return_val.if({ // beat has changed
				action.notNil.if({
					action.value(this)
				});
			});
