MidiMangler {
	...
	// I had an interesting set of weird bugs in the FisherPriceRecords class
			...
			degs.do({|d, i|
				...
				{ flag.not && (index < allowed_degrees.size) }. while({
					...
					(d == allowed_degrees[index]).if({
						...
						i = 1;
						{(i < 5) && flag.not}.while({
							allowed_degrees.indexOf(d+(12* i)).notNil.if({
