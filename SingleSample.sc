SingleSample {
	...
	var <buf, max_right, <>max_len, <conductor, pbind, <name, shiftarr, durarr, locator,
		micro_grains, <>bbcut, evt, <>continious, events, flag;
	...
	*mono { arg server, name, path, max_len = 16, max_right = 3, 
	...
	init_alloc {arg srv, name, numChannels = 1, max_len = 16, max_right = 3, 
		...
		buf = BufferTool.alloc(srv, /*44100 * */ max_len, numChannels, class:Buffer, 
				action: {|b|
	...
	*/
	...
	pbind { arg instrument, timer, multiple = 1, group ... pairs;
		...
		timer.notNil.if({
			pbind = Ptpar([timer.tempo, Pbind.new(*list)]);
		
			pbind = pbind.playExt(timer.clock);
			//Routine({pbind = pbind.play}).play(timer.tempoclock);
		} , {
			pbind = Pbind.new(*list).play;
		});
		pbind.isNil.if({("wtf").postln;});
		//pbind.trace.play;
		^pbind
	}		
	
		
	pattern {arg patt = Pbind, instrument, timer, multiple = 1, group ... pairs;
		
		var list;
		
		list = this.eventPairs(nil, timer, multiple = 1, group, *pairs);
		
		timer.notNil.if({
			pbind = Ptpar([timer.tempo, patt.new(*list)]);
		
			pbind = pbind.playExt(timer.clock);
			//Routine({pbind = pbind.play}).play(timer.tempoclock);
		} , {
			pbind = patt.new(*list).play;
		});
	...
	pmono { arg instrument, timer, multiple = 1, group ... pairs;
		...
		timer.notNil.if({
			pbind = Ptpar([timer.tempo, Pmono.new(instrument, *list)]);
		
			pbind = pbind.playExt(timer.clock);
			//Routine({pbind = pbind.play}).play(timer.tempoclock);
		} , {
			pbind = Pmono.new(instrument, *list).play;
		});
	...
	*/
	...
	startBBCut{ |timer, offset, cutproc|
		...
		bbcut = BBCut2(CutBuf3(buf.buf, offset), cutproc).play(timer.clock);
		^bbcut;
	}
